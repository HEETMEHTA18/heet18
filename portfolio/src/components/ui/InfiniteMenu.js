...
void main() {
    int itemIndex = vInstanceId % max(uItemCount, 1);
...
class DiscGeometry extends Geometry {
  constructor(steps = 4, radius = 1) {
    super();
    const localSteps = Math.max(4, steps);
    const alpha = (2 * Math.PI) / localSteps;
...
class ArcballControl {
  constructor(canvas, updateCallback) {
    ...
    canvas.addEventListener('pointerdown', this._listeners.pointerdown);
    canvas.addEventListener('pointerup', this._listeners.pointerup);
    canvas.addEventListener('pointerleave', this._listeners.pointerleave);
    canvas.addEventListener('pointermove', this._listeners.pointermove);

    canvas.style.touchAction = 'none';
  }

  destroy() {
    this.canvas.removeEventListener('pointerdown', this._listeners.pointerdown);
    this.canvas.removeEventListener('pointerup', this._listeners.pointerup);
    this.canvas.removeEventListener('pointerleave', this._listeners.pointerleave);
    this.canvas.removeEventListener('pointermove', this._listeners.pointermove);
  }
...
class InfiniteGridMenu {
  ...
  run = (time = 0) => {
    const deltaTime = Math.min(32, time - this.time || this.TARGET_FRAME_DURATION);
    this.time = time;
    this.frames += deltaTime / this.TARGET_FRAME_DURATION;
  ...
  init(onInit) {
    ...
    if (!gl) {
      throw new Error('WebGL2 is required for InfiniteMenu.');
    }
  ...
  render() {
    ...
    gl.uniform1i(this.discLocations.uAtlasSize, Math.max(1, this.atlasSize));
  ...
  updateProjectionMatrix(gl) {
    this.camera.aspect = gl.canvas.clientWidth / Math.max(gl.canvas.clientHeight, 1);
  ...
  onControlUpdate(deltaTime) {
    ...
    if (!this.control.isPointerDown) {
      ...
      const itemCount = Math.max(1, this.items.length);
      this.onActiveItemChange(nearestVertexIndex % itemCount);
...
const defaultItems = [
  {
    ...
    description: 'Spin to explore projects',
  },
];
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  useEffect(() => {
    ...
    sketch = new InfiniteGridMenu(canvas, safeItems, handleActiveItem, setIsMoving, (instance) => {
      instance.run();
    }, scale);
